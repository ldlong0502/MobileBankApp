...
const HomeScreen = () => {
        ...
        querySnapshot.docChanges().forEach(change => {
          ...
          if (change.type === 'modified') {
            ...
            if (change.doc.data().surplus > Data.getDataUser.surplus) {
              body =
                'Tài khoản ' +
                Data.getDataUser.bankID +
                ' tại LLTB +' +
                (change.doc.data().surplus - Data.getDataUser.surplus) +
                ' VNĐ vào lúc ' +
                HelpFunction.getCurrentDate() +
                '. Số dư: ' +
                change.doc.data().surplus +
                ' VNĐ';
            } else {
              body =
                'Tài khoản ' +
    ...
    return (
      <View
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <TouchableOpacity
            onPress={() => Clipboard.setString(Data.getDataUser.bankID)}
        ...
        <View style={{flexDirection: 'row'}}>
          <Text style={{flex: 1, color: '#DFF6FF', margin: 20, ...FONTS.body2}}>
            {t('common:surplus')}:{' '}
            {see ? HelpFunction.formatMoney(surplus) : 'xxxxxxxxx VNĐ'}
          </Text>
          <TouchableOpacity
            onPress={() => setSee(!see)}
            style={{alignSelf: 'center'}}>
            <Image
              source={see ? icons.eye : icons.disable_eye}
              style={{
                flex: 0,
                height: 20,
                width: 20,
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor:'#DFF6FF'}}>
      {renderPromos()}
    </SafeAreaView>
  );
};
