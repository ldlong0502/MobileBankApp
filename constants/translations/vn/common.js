export default {
  //Transaction
  titleTransaction: 'Lịch sử giao dịch',
  surplus: 'Số dư',
  transaction: 'Giao dịch',
  sort: 'Sắp xếp',
  today: 'Hôm nay',
  search: 'Tìm kiếm theo ngày',
  in: 'VÀO',
  out: 'RA',
  inAlert: 'Tổng tiền bạn nhận được tháng này: ',
  outAlert: 'Tổng tiền bạn đã tiêu tháng này: ',
  total: 'Tổng cộng',

  //SettingScreen
  exit: 'Thoát',
  welcome: 'Kính chào quý khách',
  personal: 'Cá nhân',
  changeAvatar: 'Đổi ảnh đại diện',
  changeBackground: 'Đổi ảnh nền',
  advancedSetting: 'Cài đặt nâng cao',
  smartOTP: 'Đổi mã Pin',
  phoneBook: 'Danh bạ chuyển tiền',
  paymentForm: 'Mẫu thanh toán',
  changePassword: 'Đổi mật khẩu',
  otherSetting: 'Cài đặt khác',
  contact: 'Liên hệ chúng tôi',
  privacy: 'Chính sách bảo mật',
  //Other Setting
  setting: 'Cài đặt',
  languageSetting: 'Cài đặt ngôn ngữ',
  chooseLanguage: 'Chọn ngôn ngữ',
  english: 'Tiếng Anh',
  vietnamese: 'Tiếng Việt',

    //ChangeAvatar
  fromCamera: 'Từ Camera',
  fromLibrary: 'Từ Thư viện',
  cancel1: 'Hủy',
  editAvatar: 'Chỉnh sửa ảnh đại diện',
  //ChangeBackground
  takePhoto: 'Chụp ảnh',
  loadFromLibrary: 'Tải lên từ thư viện',
  imageAvailable: 'Ảnh có sẵn',
  cancel2: 'Hủy bỏ',

  //ChangePassword
  oldPassword: 'Nhập mật khẩu cũ',
  newPassword: 'Nhập mật khẩu mới',
  againNewPassword: 'Nhập lại mật khẩu mới',
  complete: 'Hoàn tất',
  passTitle1: 'Mật khẩu phải thỏa mãn các điều kiện sau:',
  passTitle2: '- Có độ dài từ 8 đến 20 ký tự\n- Chứa ít nhất 01 ký tự số, 01 ký tự chữ và 01 ký tự đặc biệt\n\n Ví dụ: b&123456;B&123456;&B123456',

   //Image available
  chooseBackGround: 'Chọn ảnh nền',
  save: 'Lưu',
  //BottomBars
  home: 'Trang chủ',
  payment: 'Thanh toán',

    //Home
  titleHome: 'Chào mừng đến!',
  recharge: 'Nạp tiền',
  transfer: 'Chuyển tiền',
  scan: 'Quét mã QR',
  wallet: 'Ví',
  bill_Payment: 'Thanh toán hóa đơn',
  internet_Payment: 'Thanh toán Internet',
  phone_Payment: 'Thanh toán điện thoại',
  more: 'Thêm',
  feature: 'Tính năng',
  accountInformation: 'THÔNG TIN',
  numAccount: 'Số TK',
  service: 'Dịch vụ yêu thích',
  food: 'Đặt thức ăn',
  foodBody: 'Cùng ăn nào!!!',
  gift: 'Quà sinh nhật',
  giftBody: 'Chúc mừng sinh nhật',
  hotel: 'Đặt HomeStay',
  hotelBody: 'Cùng du lịch nào!!!',
  invest: 'Đầu tư cổ phiếu',
  investBody: 'Đầu tư thông minh',
  viewAll: 'Xem tất cả',

  //Login
  phone: 'Số điện thoại',
  password: 'Mật khẩu',
  login: 'Đăng nhập',
  forgotPassword: 'Quên mật khẩu?',
  loginTitlte1: 'Chưa có tài khoản? Đăng ký',

  //Forgot Pass
  forgotPasswordTitle: 'Khôi phục mật khẩu',
  send: 'Gửi',
  back: 'Trở về Đăng nhập',

  // Sign Up
  signUpPasswordTitle: 'Tài khoản mới',
  username: 'Tên người dùng',
  inputPassword: 'Nhập mật khẩu',
  inputAgainPassword: 'Nhập lại mật khẩu',
  otp: 'Nhập OTP',
  signUp: 'Đăng ký',
  ruleSignUp: 'Bằng cách đăng ký, bạn xác nhận rằng bạn chấp nhận',
  and: 'và',
  termsOfUse: 'Điều khoản sử dụng',
  privacyPolicy: 'Chính sách bảo mật',
  loginBack: 'Đã có tài khoản? Đăng nhập',


   // Phone payment
   
   title1: 'TRẢ TRƯỚC',
   title2: 'MUA MÃ THẺ',
   title3: 'Chọn mệnh giá',
   title4: 'Nhập số điện thoại',
   title5: 'Mua ngay',
   cped: 'Đã sao chép',
   pcc: 'Mã thẻ'
};
